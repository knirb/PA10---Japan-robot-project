...
void PA10Controller::MoveJoint()
{
  ...
  for(int i=0; i<7; ++i)
    {
      if (i<2) {
        cur_joint.position[i]=a_0[i]*ticks + a_1[i]*pow(ticks,2) + a_2[i]*pow(ticks,3) + a_3[i]*pow(ticks,3) + a_4[i]*pow(ticks,4) + a_5[i]*pow(ticks,5);
      }else if (i > 2) {
        cur_joint.position[i]=a_0[i-1]*ticks + a_1[i-1]*pow(ticks,2) + a_2[i-1]*pow(ticks,3) + a_3[i-1]*pow(ticks,3) + a_4[i-1]*pow(ticks,4) + a_5[i-1]*pow(ticks,5);
      }
    }

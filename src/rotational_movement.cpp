...
private:
  ...
  double start[6] = {0, 90, 0, 0, 0, 0};
  double target[6] = {0, 0, 0, 0, 0, 0};
  vector<double> radius;
  vector<double> circleNormal;
...
//Constructor
  PA10Controller::PA10Controller()
  {
    ...
    nParts = 100;
    nPoints = nParts + 1;
    radius =vector<double>(1);
    radius[0] = 300.0;
    circleNormal = vector<double>(3);
    center = vector<double>(3);
    circleNormal[0] = 1.0;
    circleNormal[1] = 0.0;
    circleNormal[2] = 0.0;
    center[0] = 550.0;
...
void PA10Controller::GeneratePoints() //Generates points for linear movement
{
    ...
    for (int i = 0; i < nPoints; ++i) {
      for (int j = 0; j < fKinStart.size(); j++) {
        if (j == 3) {
          trajPoints[i][j] = center[0];
        }
        else if (j == 7) {
          trajPoints[i][j] = center[1] + radius[0]*cos(2*pi*i/nParts);
        }
        else if (j == 11) {
          trajPoints[i][j] = center[2]- radius[0]*sin(2*pi*i/nParts);
        }else {
          trajPoints[i][j] = fKinStart[j];
          /*if (i>=nParts/2) {
            if (j==8 || j == 5) {
              trajPoints[i][j] = -trajPoints[i][j];
            }
          }*/
        }
        //cout<<trajPoints[i][j] <<endl;
      }
      //cout<<endl;

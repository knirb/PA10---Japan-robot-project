...
private:
  ...
  double start[6] = {-80, 45, 40, 90, 90, 0};
  double target[6] = {80, 45, 40, -90, 90, 0};
...
void PA10Controller::MoveJoint(int j)
{
  ...
  double ftx = a_0[j][0] + a_1[j][0]*pow(ticks,1) + a_2[j][0]*pow(ticks,2) + a_3[j][0]*pow(ticks,3) + a_4[j][0]*pow(ticks,4) + a_5[j][0]*pow(ticks,5);
  double fty = a_0[j][1] + a_1[j][1]*pow(ticks,1) + a_2[j][1]*pow(ticks,2) + a_3[j][1]*pow(ticks,3) + a_4[j][1]*pow(ticks,4) + a_5[j][1]*pow(ticks,5);
  double ftz = a_0[j][2] + a_1[j][2]*pow(ticks,1) + a_2[j][2]*pow(ticks,2) + a_3[j][2]*pow(ticks,3) + a_4[j][2]*pow(ticks,4) + a_5[j][2]*pow(ticks,5);
  ...
  curPos[3] = ftx;
  curPos[7] = curPos[7] + 757.806*2*fty;
  curPos[11] = ftz;

...
private:
  ...
  double start[6] = {-80, 45, 30, 90, 90, 0};
  double target[6] = {80, 45, 30, 90, -90, 0};
...
//Constructor
  PA10Controller::PA10Controller()
  {
    ...
    trajPoints = std::vector< std::vector<double> >(nPoints+1, std::vector<double>(16));
    trajAngles = std::vector<sensor_msgs::JointState>(nPoints+1);
...
std::vector<double> PA10Controller::ForwardKinematics(sensor_msgs::JointState joint)
{
  double Ti[4][4];
  double tempRes[4][4];
  double theta[6] = {joint.position[0]*180/pi,joint.position[1]*180/pi,joint.position[3]*180/pi,joint.position[4]*180/pi,joint.position[5]*180/pi,joint.position[6]*180/pi}; //Checked to be correct
  for (int i = 0; i < 6; i++) {
...
void PA10Controller::GeneratePoints() //Generates points for linear movement
{

    // T matrices for Start, end + delta vector to be divided into nParts points.
    std::vector<double> fKinStart(16);
...
void PA10Controller::GenerateAngles() //Generates joint states for the points
{
  for (int i = 0; i < nPoints; ++i) {
    //cout<<trajPoints[i]
    trajAngles[i] = InverseKinematics(trajPoints[i]);
    //cout<<trajAngles[i].position[0]<<endl;
  }
}
...
void PA10Controller::PathGenerate()
{
  ...
  for (int i = 1; i < nParts; i++) {
    for (int j = 0; j < 6; ++j) {
      if (j<2){
        theta[i][j]=trajAngles[i].position[j];

      }else{
        theta[i][j]=trajAngles[i].position[j+1];
      }
    }
  }
  for (int i = 1; i < nParts; i++) {
    for (int j = 0; j < 6; ++j) {
      if (sgn((theta[i][j]-theta[i-1][j])) == sgn((theta[i+1][j]-theta[i][j])/Ti) ) {
        thetad[i][j]= ((theta[i][j]-theta[i-1][j])/Ti + (theta[i+1][j]-theta[i][j])/Ti)/2;
      }else{
        thetad[i][j]= 0;
      }
      thetadd[i][j] = 0;
  ...
  for (int i = 0; i < nParts; i++) {
    for (int j = 0; j < 6; j++) {
      ...
      a_4[i][j] = (1/(2*pow(T,4)))*(30*(theta[i][j]-theta[i+1][j])+(14*thetad[i+1][j] + 16*thetad[i][j])*T + (3*thetadd[i+1][j]-2*thetadd[i][j])*T*T);
      a_5[i][j] = (1/(2*pow(T,5)))*(12*(theta[i+1][j]-theta[i][j])-6*(thetad[i+1][j] + thetad[i][j])*T - (thetadd[i+1][j]-thetadd[i][j])*T*T);
    }
  }
}
...
{
  for(int i=0; i<7; ++i){
    cout<<trajAngles[j].position[i]*180/pi<<endl;
  }
  if (j == nParts) {
    j--;
  }
  for(int i=0; i<7; ++i)
    {
      if (i<2) {
        cur_joint.position[i]=a_0[j][i] + a_1[j][i]*pow(ticks-j*T/nParts,1) + a_2[j][i]*pow(ticks-j*T/nParts,2) + a_3[j][i]*pow(ticks-j*T/nParts,3) + a_4[j][i]*pow(ticks-j*T/nParts,4) + a_5[j][i]*pow(ticks-j*T/nParts,5);
      }else if (i > 2) {
        cur_joint.position[i]=a_0[j][i-1] + a_1[j][i-1]*pow(ticks-j*T/nParts,1) + a_2[j][i-1]*pow(ticks-j*T/nParts,2) + a_3[j][i-1]*pow(ticks-j*T/nParts,3) + a_4[j][i-1]*pow(ticks-j*T/nParts,4) + a_5[j][i-1]*pow(ticks-j*T/nParts,5);
      }
    }
    cout<<endl;
